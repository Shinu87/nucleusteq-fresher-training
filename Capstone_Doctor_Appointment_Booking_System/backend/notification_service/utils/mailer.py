"""
This is the only file in Notification Service that actually talks to a
real mail server (Gmail SMTP).

I use fastapi-mail since it already handles the async SMTP stuff for
us instead of us having to write raw smtplib code.
"""
...
async def send_email(recipient_email: str, subject: str, html_body: str) -> None:
    ...
    This does NOT catch any errors itself - if the SMTP server is down,
    the credentials are wrong, etc, this will raise an exception and the
    caller (notification_service.py) decides what to do about it.
    """
