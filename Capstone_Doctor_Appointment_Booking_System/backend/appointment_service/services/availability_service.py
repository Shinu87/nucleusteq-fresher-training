...
async def _ensure_doctor_exists(doctor_id: PydanticObjectId) -> None:
    """
    Sanity check before letting a doctor create a slot: their Doctor
    read-model row has to exist here first. If it's missing, the sync
    probably failed and an admin needs to look into it.
    """
