...
patient or admin token gets a 403. The doctor_id always comes from the
JWT (current_user.id), never from anything the caller types in, so a
doctor can never manage anyone's slots but their own.
"""
...
from appointment_service.schemas.request.slot_request import CreateSlotRequest, UpdateSlotRequest
from appointment_service.schemas.response.slot_response import SlotResponse
from appointment_service.services import availability_service
