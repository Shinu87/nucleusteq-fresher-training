...
const candidateRules = [
  ...
  {
    id: "cDob", label: "Date of Birth", required: true, type: "date",
    custom: (value) => {
      ...
      if (age < 18) return "Candidate must be at least 18 years old to apply for jobs";
      if (age > 60) return "Candidate exceeds maximum eligible working age";
      return null;
    },
  },
  { id: "cTotalExp", label: "Total experience", required: true, type: "number", min: 0, max: 60 },
  {
    id: "cRelExp", label: "Relevant experience", type: "number", min: 0, max: 60,
    watches: ["cTotalExp"],
    custom: (value) => {
  ...
  {
    id: "cResumeUrl", label: "Resume URL",
    custom: (value) => {
      if (!value) return null;
      try { new URL(value); } catch { return "Resume URL must be a valid URL (https://…)."; }
      return null;
    },
  },
...
async function createCandidate() {
  ...
  if (!name || !email || !phone || !dobRaw || !totalExp || !jobId) {
    showMsg(msgEl, "error", "Fill all required fields (Name, Email, Phone, Date of Birth, Total Exp, Job).");
    return;
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
  ...
  if (!/^[6-9][0-9]{9}$/.test(phone)) {
    showMsg(msgEl, "error", "Phone must be a 10-digit number starting with 6-9.");
    return;
  }

  // Age eligibility check
  const dob = new Date(dobRaw), today = new Date();
  if (dob >= today) { showMsg(msgEl, "error", "Date of birth must be in the past."); return; }
  let derivedAge = today.getFullYear() - dob.getFullYear();
  const monthDiff = today.getMonth() - dob.getMonth();
  if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < dob.getDate())) derivedAge--;
  if (derivedAge < 18) { showMsg(msgEl, "error", "Candidate must be at least 18 years old to apply for jobs"); return; }
  if (derivedAge > 60) { showMsg(msgEl, "error", "Candidate exceeds maximum eligible working age"); return; }
...
function clearCandidateForm() {
  ["cName", "cEmail", "cPhone", "cDob", "cOrg", "cTotalExp", "cRelExp",
   "cCurCTC", "cExpCTC", "cNotice", "cPrefLoc", "cSource", "cResumeUrl"]
...
async function viewCandidate(id) {
  try {
    ...
    let feedbacks = [];
    try { feedbacks = await apiGet(`/feedbacks/candidate/${id}`); } catch (_e) { feedbacks = []; }

    const card    = document.getElementById("candidateDetailCard");
    const content = document.getElementById("candidateDetailContent");
    const jobTitle = allJobs.find(j => j.id === c.jobId)?.title || `Job #${c.jobId}`;
    ...
    content.innerHTML = `
      ...
      <div class="detail-section">
        ...
        ${c.resumeUrl
          ? `<p style="margin-top:10px;font-size:13px;">📄 Resume:
             <a href="${BASE_URL}/resumes/download?path=${encodeURIComponent(c.resumeUrl)}" target="_blank">View Resume</a></p>`
          : ""}
      </div>
      <div class="detail-section">
...
function computeFeedbackInfo(candidate, interviews, feedbacks) {
  ...
  const submitted = (feedbacks || []).filter(f => f.interviewId === interview.id).length;
  return { gated: true, complete: assigned > 0 && submitted >= assigned, assigned, submitted };
}
...
function stageAdvanceDropdown(c, size = "sm", feedbackInfo = null) {
  ...
  const selectId = `stageSel_${c.id}`, btnId = `stageBtn_${c.id}`;
  const btnClass   = size === "sm" ? "btn btn-sm btn-success" : "btn btn-success";
  const selectStyle = size === "sm"
  ...
  const optionRows = ALL_STAGES.map((stage, i) => {
    const isCurrent = stage === c.currentStage, isNext = stage === next;
    let label = stage === "SELECTED" ? "SELECTED (final)" : stage;
    if (isCurrent)     label += "  — current";
    else if (isNext)   label += stage === "SELECTED" ? "  — mark as final" : "  — next";
    else if (i < curIdx) label += "  — past";
    else               label += "  — locked";
    return `<option value="${stage}" ${isNext ? "" : "disabled"}>${label}</option>`;
  }).join("");

  const feedbackBlocks = feedbackInfo && feedbackInfo.gated && !feedbackInfo.complete;
...
function onStageDropdownChange(selectId, btnId, lockedFlag) {
  ...
  btn.title = hasValidSelection ? "Click to advance the candidate" : "Select a stage above to enable";
}
...
async function submitStageAdvance(id, selectId, btnId, currentStage) {
  ...
  if (!target) { showToast("Please select the next stage.", "error"); return; }
  if (target !== expected) {
    showToast(`Invalid transition: ${currentStage} → ${target}. Only ${expected} is allowed.`, "error");
    return;
  }

  const promptMsg = target === "SELECTED"
  ...
  try {
    ...
    showToast(target === "SELECTED" ? "Candidate marked as SELECTED!" : `Moved to ${target}!`, "success");
    sel.value = "";
    loadCandidates();
    const card = document.getElementById("candidateDetailCard");
